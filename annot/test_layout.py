...
# a tall lateral-shaped film, like BUU. Enough of them that /peek always has something to
# prefetch: an exhausted queue is a legitimate 404, but it lands in the console as an
# error and would mask a real one.
for i in range(10):
    cid = f"film{i:03d}"
...
with sync_playwright() as p:
    ...
    # ?tool=circle: the layout and PACS checks below drive the circle primitive, which is
    # no longer the default. The arc primitive gets its own section at the end.
    pg.goto(f"http://127.0.0.1:{PORT}/?tool=circle", wait_until="networkidle")
    ...
    pgA.on("console", lambda m: aerrs.append(f"console.error: {m.text}")
           if m.type == "error" else None)
    pgA.goto(f"http://127.0.0.1:{PORT}/?tool=arc", wait_until="networkidle")
